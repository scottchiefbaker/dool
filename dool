...
XDG_PATH = os.getenv('XDG_DATA_HOME') or os.path.expanduser('~/.local/share')

DOOL_PLUGIN_PATH = [
    os.path.expanduser('~/.dool/'),                           # ~/.dool/
    XDG_PATH + '/dool/plugins/',                              # ~/.local/share/dool/plugins
...
class Options:
    def __init__(self, args):
        ...
        #
        # Commented out on 2025-08-21 as we don't have any optional param plugins
        # right now
        ...
        try:
            opts, args = getopt.gnu_getopt(args, 'acdfghilmno:prstTvyC:D:I:M:N:S:V', all_opts)
    ...
    def optional_params(self, args):
        optional_arguments = [] # Plugins with opt params go in here
        to_remove          = []
        ...
        # If an item has an option argument and we've consumed it
        # we remove it from the args list
        args = remove_array_items(args, to_remove)
    ...
    def get_opt_list_from_details(self, plugin_details):
        ...
        for name in plugin_names:
            ...
            opts  = plugin_details[name]['opts'] # Plugin needs CLI params
            ptype = plugin_details[name]['type'] # builtin or exteral

            param_req = "PARAM_REQUIRED" in opts
            ...
            # print("%15s = %s (param required: %s)" % (name, path, param_str))
    ...
    def version(self):
        print(text_color(87, 'Dool ' + __version__))
        ...
        print('Ticks per sec: %d' % os.sysconf('SC_CLK_TCK'))
        print()
...
class dool_proc(dool):
    ...
    def extract(self):
        ...
        # The number of new processes is the average number of processes since the last time
        # we outputted the stats
        ...
        # Triggers at the start of a new grouping of delay seconds
        if step == op.delay:
            # Store set1 as the basis for the new averages
            self.set1.update(self.set2)

            # Clear the set2 counters and start fresh
            for name in ('procs_running', 'procs_blocked'):
                self.set2[name] = 0
...
def devel_log(msg):
    ...
    if DEBUG_LAST > 0:
       time_diff = (time_ns - DEBUG_LAST) / 1000000.0 # NS -> Milliseconds
    else:
       time_diff = 0

    # Print the formatted line to the log file
    log_line = ("%s %7.2f ms: %s\n" % (time_str, time_diff, msg.strip()))
...
def get_name_by_proc_cmdline(pid):
    ret = ""

    try:
        proc_file = ('/proc/%s/cmdline' % pid);
        parts     = linecache.getline(proc_file, 1).split('\0')
        first     = list_item_default(parts, 0, "")
        ret       = basename(first)
        # devel_log("CMDLINE/PID: %s => '%s'" % (pid, ret));
    except:
        pass # Do nothing
...
def get_name_by_proc_comm(pid):
    ret = ""

    try:
        proc_file = ('/proc/%s/comm' % pid);
        parts   = linecache.getline(proc_file, 1).split('\0')
        first   = list_item_default(parts, 0, "").strip()
        ret     = basename(first)

        # devel_log("COMM/PID: %s => '%s'" % (pid, ret));
    except:
        pass # Do nothing
...
def get_name_by_proc_stat(pid):
    ...
    try:
        ...
        # devel_log("STAT/PID: %s => '%s'" % (pid, ret));
    except:
        pass # Do nothing
...
def getnamebypid(pid, default):
    "Fallback/Deprecated wrapper for old plugins"

    # Poor mans static variable
    if not hasattr(getnamebypid, "first"):
        getnamebypid.first = 1;

    if getnamebypid.first:
        print(text_color(214, "Warning: ") + "You are using old/deprecated Dool plugins. Please upgrade")
        getnamebypid.first = 0

    return get_name_by_pid(pid)

def get_name_by_pid(pid, debug=0):
    "Return the name of a process by taking best guesses and exclusion"

    # There are three possible places PID name can live we check them in order
    ret = get_name_by_proc_cmdline(pid)
    method = "cmdline"

    if not ret:
        ret = get_name_by_proc_comm(pid)
        method = "comm"

    if not ret:
        ret = get_name_by_proc_stat(pid)
        method = "stat"

    cmd_raw = linecache.getline('/proc/%s/cmdline' % pid, 1).strip()
    cmdline = cmd_raw.split('\0')

    # Strip off any scripting language name
    if ret in ('bash', 'csh', 'ksh', 'perl', 'python', 'ruby', 'sh'):
        ret = basename(list_item_default(cmdline, 1, ""))
    # I'm not sure what this does... needs more testing/documentation
    if ret.startswith('-'):
        ret = basename(list_item_default(cmdline, -2, ""))

    # If it's a VM hypervisor we strip it down to get the VM name
...
def get_plugin_details():
    ...
    for function_name in globals():
        if function_name.startswith('dool_'):
            ...
            obj['type'] = 'builtin'
            obj['opts'] = [] # The builtins don't have any opts (right now)

            ret[name] = obj

    # The external `.py` plugins
    name_regexp = re.compile('.+/dool_?(.+).py$')
    external    = []

    # We cheat here because we haven't parsed the options array so op.debug is not
    # available yet
    has_debug = "--debug" in sys.argv

    for path in DOOL_PLUGIN_PATH:
        # Plugins must be named dool_NAME.py
        for filename in glob.glob(path + '/dool_[a-zA-Z0-9]*.py'):
            plugin_name = name_regexp.match(filename).group(1)
            name        = plugin_name.replace('_', '-')
            ...
            if (ret.get(name)):
                if has_debug:
                    k("Skipping adding %s: '%s' because it's a dupe name" % (name, filename))
            else:
                # See if there are any special options specified in the plugin file
                obj['opts'] = get_plugin_file_opts(filename)
                ret[name]   = obj

    return ret

# Open up the plugin file and look for a DOOL_OPTS: STRING
def get_plugin_file_opts(file):
    # We check the first 256 bytes of the plugin for the DOOL_OPTS string
    mystr = file_slurp(file, 256)
    match = re.search(r"DOOL_OPTS:\s?(.+?)$", mystr, re.MULTILINE)
    opts  = []

    # If we find the string we break out the individual opts
    if match:
...
def show_plugins():
    ...
    term_width = cols - 22;
    indent     = "        "
    # We parse this manually because op.debug isn't available yet
    has_debug  = "--debug" in sys.argv

    # In debug mode we show which directories are parsed when looking for plugins
    if has_debug:
        plugin_dir_str = ",".join(DOOL_PLUGIN_PATH)
        lines          = text_wrap(plugin_dir_str, ',', term_width)
        print("Plugin directories:")
        for x in lines:
            print(indent + x)

    # Parse the plugins into a hash so we can print them out later
    for name in plugin_names:
    ...
    # Print out the internal plugins
    print(text_color(228, 'internal:'))
    builtin_str = ",".join(builtin)
    lines       = text_wrap(builtin_str, ',', term_width)
    for x in lines:
        print(indent + x)
    ...
    for path in DOOL_PLUGIN_PATH:
        ...
        # If no plugins are found we skip this section/directory
        if not plugins: continue

        plugins.sort()

        # Print out the comma separated list of plugins in this external directory
        dir_str = os.path.abspath(path) + ":"
        print(text_color(15, dir_str))
        plugin_str = ",".join(plugins)
        lines      = text_wrap(plugin_str, ',', term_width)
        for x in lines:
            print(indent + x)
    ...
    for plugin in op.plugins:
        ...
        try:
            if pluginfile not in globals():
                ...
                # Load the plugin body into a string and then 'exec' the code
                # into the global namespace
                plugin_str = open(pluginfile).read()
                code       = compile(plugin_str, pluginfile, 'exec')
            ...
            if op.debug >= 3:
                ...
                print('%sPlugin %s failed to load. (%s)' % (text_color(208, "Warning: "), text_color(15, pluginfile), err_str), file=sys.stderr)
...
def perform(update):
        ...
        devel_log("Starting plugins")
        # Loop through each param section and build the output string
        for o in totlist:
            o.extract()

            devel_log("Plugin: '%s'" % (plugin_obj_name(o)))
...
# Get the plugin name from the object. Could be an array or a string
def plugin_obj_name(o):
...
def list_item_default(mylist, num, default):
    # If it's not a list, just return the default
...
def truncate_pad(input_string, length):
    """
    Truncates a string if it's longer than the specified length,
    or pads it with spaces on the right if it's shorter.

    Args:
        input_string (str): The string to truncate or pad
        length (int): The desired final length of the string

    Returns:
        str: The truncated or padded string of exactly 'length' characters
    """

    ret = ""
    if len(input_string) > length:
        # Truncate if longer
        ret = input_string[:length]
    else:
        # Pad with spaces if shorter
        ret = input_string.ljust(length)

    return ret

def text_wrap(text, delim, width=80):
    parts        = text.split(delim)
    lines        = []
    current_line = ""

    for part in parts:
        # Add back the comma (except for the very last part)
        segment = part + ","
...
def k(obj, prefix = "DEBUG", filename = "", line_num = 0):
    import inspect
    ...
    if (filename):
        ...
        info     = inspect.getframeinfo(stack[0])
        myfile   = os.path.basename(info.filename)
...
# Krumo print and die
def kd(obj, prefix = "DIED"):
    import inspect

    # We pass in the file/line_num because the kd() ends up in the call stack
    stack    = inspect.stack()[1]
    info     = inspect.getframeinfo(stack[0])
    line_num = info.lineno
    myfile   = os.path.basename(info.filename)

    k(obj, prefix, myfile, line_num)

    if (info.filename != __file__):
        raise Exception("KRUMO_DIE")
...
# Safely remove items from a list while handling edge cases and maintaining the original list order.
def remove_array_items(source_list, items_to_remove):
    # Convert items_to_remove to a set for O(1) lookup
    ...
    for item in source_list:
        # Only keep items that are not in the remove set
...
def extract_between_parens(text, start_delim = '(', end_delim = ')'):
    ...
    # Add 1 to start_index to exclude the opening parenthesis
    ret = text[start_index + 1:end_index]
    return ret

# Loop through an array of files and return the first one on the filesystem
def first_existing_file(files):
    """
    Return the first file from the array that exists on the filesystem.
    Returns None if no files exist.
    """
    ret = next((f for f in files if os.path.exists(f)), None)
