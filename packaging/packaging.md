...
* The `dool.1` manpage should be placed the appropriate manpage location.
This is usually `/usr/share/man/man1` (optional)

Plugin installation can be verified by running `dool --list` to show all the
plugins that were found and what directories they were found in. If you are
having problems packaging the plugins they can be skipped. 90% of the
functionality is built in to the core `dool` script.
...
Dool uses **make** to build packages. In the `packaging/` directory we break
out each platform with an associated build script.
